...
from fpc_dataset import FPCDataSet
...
def main():
    ...
    root = "./"
    # check voc root
    if os.path.exists(os.path.join(root, "VOCdevkit")) is False:
        raise FileNotFoundError("VOCdevkit dose not in path:'{}'.".format(root))

    # load train data set
    train_data_set = FPCDataSet(root, data_transform["train"], True)
    ...
    val_data_set = FPCDataSet(root, data_transform["val"], False)
